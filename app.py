...
with st.sidebar:
    st.header("Acerca de")
    st.caption("Versión 1.1 · Autor: Enteza-CarrySoft · Procesamiento local en memoria.")
    detailed = st.toggle("Modo detallado (logs)")

# Formulario
with st.form("form_xsig"):
    file = st.file_uploader(
        "Sube el archivo XSIG",
        type=["xsig", "xml"],
        help="Tamaño máximo recomendado: 20 MB"
    )
    num_registro = st.text_input(
        "Número de Registro",
...
if submitted:
    ...
    try:
        ...
        # Botón de descarga
        suggested_name = make_safe_filename(
            f"Factura_{num_rcf_s or num_registro_s}_{fecha_hora_registro_dt.strftime('%Y%m%d_%H%M')}.pdf"
        )
        st.download_button(
            label="Descargar PDF",
